...
if (isset($_POST['addProduct'])) {
  ...
  if (empty($_POST["prod_code"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price']) || empty($_POST['category'])) {
    ...
    $postQuery = "INSERT INTO rpos_products (prod_id, prod_code, prod_name, prod_img, prod_desc, prod_price, fk_cat_id) VALUES(?,?,?,?,?,?,?)";
    $postStmt = $mysqli->prepare($postQuery);
    if ($postStmt) {
      //bind parameters
      $rc = $postStmt->bind_param('ssssssi', $prod_id, $prod_code, $prod_name, $prod_img, $prod_desc, $prod_price, $category_id);
      if ($rc) {
        $postStmt->execute();
        ...
        $err = "Failed to bind parameters: " . $mysqli->error;
      }
    } else {
...
$categoryQuery = "SELECT * FROM rpos_categories";
$categoryResult = $mysqli->query($categoryQuery);
$categoryOptions = "";
while ($category = $categoryResult->fetch_assoc()) {
  $categoryOptions .= "<option value='{$category['cat_id']}'>{$category['cat_name']}</option>";
}
